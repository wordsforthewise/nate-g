...
```{r top100_products, warning = F, message = F}
...
```{r make_data_table, results = 'hide'}
train$id <- 0
...
```{r plot_freqs, message = F, results = 'hide'}
...
```{r plot_target_again, message = F, warning = F, results = 'hide'}
ggplot(data, aes(x = target)) + geom_histogram() +
